 ...
 */
size_t rhf::HOMO_index(unsigned K, unsigned N) {
    if (N % 2 != 0) {
 ...
 */
size_t rhf::LUMO_index(unsigned K, unsigned N) {
    if (N >= 2 * K) {
 ...
 */
Eigen::MatrixXd rhf::calculate_P(Eigen::MatrixXd& C, unsigned N) {
 ...
 */
Eigen::MatrixXd rhf::calculate_G(Eigen::MatrixXd& P, Eigen::Tensor<double, 4>& tei_tensor) {
 ...
 *
 */
double rhf::calculate_electronic_energy(Eigen::MatrixXd& P, Eigen::MatrixXd& H_core, Eigen::MatrixXd& F) {
